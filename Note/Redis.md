...
1. 3主3从redis集群配置
   ...
   4. 链接进入6381作为切入点，查看并检验集群状态
      ...
      + cluster nodes：查询集群节点信息，redis-cli -p 6381 cluster nodes
...
缓存雪崩，是指在某一个时间段，缓存集中过期失效或者redis宕机，导致大量请求到达数据库，带来巨大压力
...
2. 高一致性需求：主动更新，并以超时剔除作为兜底方案
   ...
   2. 写操作：先写数据库，然后再删除缓存，要确保数据库和缓存操作的原子性
...
+ key中成员的数据量过大：一个hash类型的key，他的成员数量虽然只有1000个，但是这些成员的value总大小为100MB
...
+ 网络阻塞：少量的QPS就可能导致带宽使用率被占满，导致redis实例乃至物理机变慢
...
+ IO多路复用
  ...
  select和poll只会通知用户进程有FD(文件描述符)就绪，但不确定是哪个FD，需要用户进程逐个遍历FD来确认
